...
#contact view 
def contact(request):
    return render(request, 'student_dashboard.html', {'email': 'your_email@example.com'})
...
def update_hours(request):
    if request.method == 'POST':
        form = LibraryHoursForm(request.POST)
        if form.is_valid(): 
            form.save()
            return redirect('update_hours.html')  
    else:
        form = LibraryHoursForm()
    
    return render(request, 'update_hours.html', {'form': form})

@login_required
def student_dashboard(request):
    last_updated_hour = LibraryHours.objects.order_by('-id').first()

    # Debug to ensure data is fetched correctly
    if last_updated_hour:
        print(f"DEBUG: Last updated hour - Day: {last_updated_hour.day_of_week}, "
              f"Opening: {last_updated_hour.opening_time}, "
              f"Closing: {last_updated_hour.closing_time}")
    else:
        print("DEBUG: No library hours found in the database.")

    return render(request, 'student_dashboard.html', {'last_updated_hour': last_updated_hour})

@login_required
def library_hours(request):
    # Fetch the most recently updated entry
    last_updated_hour = LibraryHoursForm.objects.order_by('-id').first()

    # Debugging output to ensure the data is fetched
    if last_updated_hour:
        print(f"DEBUG: Last updated hour fetched - {last_updated_hour.day_of_week}, "
              f"{last_updated_hour.opening_time}, {last_updated_hour.closing_time}")
    else:
        print("DEBUG: No library hours found in the database.")

    # Pass the last updated hour as a list to maintain the template structure
    return render(request, 'update_hours.html', {'update_hours': [last_updated_hour] if last_updated_hour else []})

def room_list(request):
    rooms_list = Rooms.objects.all()  # שליפת כל החדרים
    return render(request, 'Availablerooms.html', {'rooms': rooms_list})

@login_required
def update_room_availability(request):
    if request.method == "POST":
        if "action" in request.POST:
            ...
            if action == "add_room":
                ...
                new_room_status = request.POST.get("new_room_status")
                new_room_capacity = int(request.POST.get("new_room_capacity"))

                Rooms.objects.create(
                    name=new_room_name,
                    Closed=new_room_status,
                    people=new_room_capacity
                )
            elif action == "update_rooms":
                # Updating existing rooms
                for key, value in request.POST.items():
                    if key.startswith("room_id_"):
                        room_id = int(value)
                        room_status = request.POST.get(f"room_status_{room_id}")
                        room_capacity = request.POST.get(f"room_capacity_{room_id}")

                        # Update the room in the database
                        room = Rooms.objects.get(id=room_id)
                        room.Closed = room_status
                        room.people = int(room_capacity)
                        room.save()
    ...
    return render(request, "update_rooms.html", {"rooms": rooms})
